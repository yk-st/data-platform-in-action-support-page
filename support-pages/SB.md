...
```
>>> spark.sql("DESC local_data_platform.brz_ingestion.users").show(truncate=False, n=100)
+-------------------+-----------------------+----------------------+
...
```
spark.sql("""drop table if exists local_data_platform.brz_ingestion.users""")
```
...
```
spark.sql("""
  CREATE TABLE IF NOT EXISTS local_data_platform.brz_ingestion.users_min (
    id   BIGINT,
    name STRING
  )
  USING iceberg
...
```
spark.sql("select * from local_data_platform.brz_ingestion.users_min").show()
```

spark-dataframe APIの場合

```
spark.read.table("local_data_platform.brz_ingestion.users_min").show()
```
...
```
spark.sql("""
  INSERT INTO local_data_platform.brz_ingestion.users_min
  VALUES
    (1, 'Alice'),
    (2, 'Bob'),
    (3, 'Carol');
...
```
spark.sql("""
  DELETE FROM local_data_platform.brz_ingestion.users_min
  WHERE id IN (2);
""")
```
...
spark.sql("""
  UPDATE local_data_platform.brz_ingestion.users_min
...
```
spark.sql("""
  MERGE INTO local_data_platform.brz_ingestion.users_min t
  USING src s
  ON t.id = s.id
...
s3://local-data-platform/warehouse/ref/brz_ingestion.db/users/
├─ metadata/                                  # テーブルのメタ情報
...
```
table_fqn = "local_data_platform.brz_ingestion.users"
manifest_path = (
    spark.sql(f"""
...
CALL local_data_platform.system.expire_snapshots(
  table       => 'brz_ingestion.orders_microbatch'
)
""").show()
...
![コンパクション後](images/0D/compaction_after.png)
...
spark.sql("""
  CALL local_data_platform.system.rewrite_data_files(
    table    => 'local_data_platform.brz_ingestion.orders_microbatch',
...
+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
|s3a://local-data-platform/warehouse/blz/brz_ingestion.db/orders_microbatch/data/%E4%BD%9C%E6%88%90%E6%97%A5%E6%99%82_day=2025-06-23/00083-13198-0dce8287-fac0-42c2-b8f7-819775d4982c-0-00001.parquet|
|s3a://local-data-platform/warehouse/blz/brz_ingestion.db/orders_microbatch/metadata/00000-281245aa-d0a5-42a8-add0-1198372aea25.metadata.json                                                        |
略
+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
```
...
# 直近のスナップショットを確認
spark.sql("""
  SELECT * FROM local_data_platform.brz_ingestion.users.snapshots;
...
spark.sql("""
  SELECT * FROM local_data_platform.brz_ingestion.users.refs;
""").drop("manifest_list","summary").show(truncate=False)
...
spark.sql("""
  CALL local_data_platform.system.rollback_to_snapshot(
    'brz_ingestion.users', 2660693776158621537
  )
""").show()
...
# modify ブランチを作成
spark.sql("""
  ALTER TABLE local_data_platform.brz_ingestion.users
  CREATE BRANCH IF NOT EXISTS modify
""")

# modify ブランチのデータを更新
spark.sql("""
  UPDATE local_data_platform.brz_ingestion.users.`branch_modify` AS t
...
# 確認
spark.sql("select * from local_data_platform.brz_ingestion.users.branch_modify where user_id=1").show(truncate=False)
+-------+--------------------------------------+-----------------+--------+---------+-------------------+----------+--------+-------------------+--------------------------+----------+----------+-----------+-------------------+
...
spark.sql("""
  CALL local_data_platform.system.snapshot(
    ...
    table        => 'local_data_platform.brz_ingestion.legacy_fund_master_icb',
...
spark.sql("""
  CALL local_data_platform.system.rewrite_data_files(
    table   => 'brz_ingestion.legacy_fund_master_icb',
    strategy => 'sort',
...
spark.sql("""
  SELECT snapshot_id
  FROM local_data_platform.brz_ingestion.users.snapshots
...
spark.sql("""
  SELECT * FROM local_data_platform.brz_ingestion.users
...
(df.write
   ...
   .mode("append")
   .save("local_data_platform.brz_ingestion.users_min"))
...
spark.sql("""
  ...
  FROM local_data_platform.brz_ingestion.users_min.snapshots
  ORDER BY committed_at DESC
  LIMIT 5
""").show(truncate=False)
